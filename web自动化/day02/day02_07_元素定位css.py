...
CSS常用定位方法
    1. id选择器  前提：元素是必须有id属性   语法：#id  如：#passwordA    定位id用#号
    2. class选择器  前提：元素是必须有class属性  语法：.class  如：.telA  定位class用.
    3. 元素选择器  语法：element  如：input  定位元素直接用元素名
    4. 属性选择器  语法：[属性名=属性值]  定位属性 [属性名=属性值]  同样 和正则一样 在等号前加……表示以该内柔开头 $表示以该内容结尾 *表示包含该内容
    5. 层级选择器  语法： 1. p>input    2. p input   提示：>与空格的区别，大于号必须为子元素，空格则不用。  层级定位 直接元素间空格 也可以用>表示前后顺序
扩展：
