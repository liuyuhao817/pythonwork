...
class TestLogin(unittest.TestCase):
    ...
    def test01_login_success(self):
        ...
        # 调用登录接口获取登录信息，并进行断言
        response = self.login_api.login(self.session, "17362955793", "123456", "8888")
        print(response.json())
        self.assertEqual(200, response.status_code)
    ...
    def test02_user_is_not_exist(self):
        ...
        # 调用登录接口获取登录信息，并进行断言
        response = self.login_api.login(self.session, "17362955788", "123456", "8888")
        print(response.json())
        self.assertEqual(200, response.status_code)
        self.assertEqual(-1, response.json().get("status"))
    ...
    def test03_password_error(self):
        ...
        # 调用登录接口获取登录信息，并进行断言
        response = self.login_api.login(self.session, "17362955793", "error", "8888")
        print(response.json())
        self.assertEqual(200, response.status_code)
